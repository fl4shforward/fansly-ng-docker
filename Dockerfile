...
RUN python -m pip install -r ./requirements.txt --no-cache-dir && \
    chmod -R 777 /usr/src/fansly-ng && \
    mkdir /.pyffmpeg && chmod 777 /.pyffmpeg
...
RUN sed -i 's/https:\/\/apiv3.fansly.com\/api\/v1\/group/https:\/\/apiv3.fansly.com\/api\/v1\/messaging\/groups/' /usr/src/fansly-ng/api/fansly.py && \
    sed -i "s/\['response'\]\['groups'\]/['response']['aggregationData']['groups']/" /usr/src/fansly-ng/download/messages.py
